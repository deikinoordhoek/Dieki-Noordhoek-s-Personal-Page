	...
	$.fn.lightBox = function(settings) {
		...
		function _set_image_to_view() { // show the loading
			...
			objImagePreloader.onload = function() {
				...
				if ( objImagePreloader.width > (viewportWidth - 40) ) {
        	                	  t1 = objImagePreloader.width;
        	                	  t2 = objImagePreloader.height;
        	                	  newWidth = t1 * ((viewportWidth - 40) / objImagePreloader.width);
        	                	  newHeight = t2 * ((viewportWidth  - 40) / t1);
					  if ( newHeight > (viewportHeight - 100)) {
					  	t1 = newHeight;
                        		  	t2 = newWidth;
                        		  	newHeight = t1 * ((viewportHeight - 100) / newHeight);
                        		  	newWidth = t2 * ((viewportHeight - 100) / t1);
					  };
        	                };
	                        if ( objImagePreloader.height > (viewportHeight - 100) ) {
					t1 = objImagePreloader.height;
                        		t2 = objImagePreloader.width;
                        		newHeight = t1 * ((viewportHeight - 100) / objImagePreloader.height);
                        		newWidth = t2 * ((viewportHeight - 100) / t1);
                    		};
